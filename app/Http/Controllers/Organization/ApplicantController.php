<?php
	...
	class ApplicantController extends Controller {
	    ...
	    public function index() {
	        ...
	        return view('organization.applicants.index', compact('users', 'approves', 'schedules', 'rejects'));
		}
		...
		public function show($id) {
			...
			return view('organization.applicants.information', compact('user'));
	    }
	    ...
	    public function saved() {
            ...
            return view('organization.applicants.bookmark', compact('users', 'jobs'));
	    }
	   	...
	   	public function approve(Request $request) {
	 		...
	 		try {
		        ...
		        /*if validates successful*/ 
            		...
            		$getOrganization = EmployerDetail::where('usersId', Auth::user()->id)->first();

		  			$subject2 = "Application for ".$job_name.".";
		  			$message2 = "Hi! Thank you for your attention and interest on the profile of this <b>".$receiver_name."</b>.<br><br>May we remind you about the existing rules of the Philippine Overseas Employment Administration (POEA), to recruit Filipino job candidates only through a licensed Philippine recruitment agency.<br><br>To be guided on how to proceed with the recruitment process, please contact any of the certified Philippine Sending Organization in this platform.<br><br>Yours truly,<br><b>GCC</b>";
		  			$emailArray2 = array("receiver_email" => $getOrganization->email, "receiver_name" => $getOrganization->company,"message" => $message2, "subject" 	=> $subject2);

		  			/*send the email*/
		  			EmailModel::sendEmail($emailArray2);

		  			$affilations = SubscriberAffilation::where(function ($query) {
							            $query->where('usersId', Auth::user()->id)
							                  ->orWhere('companyId', Auth::user()->id);
		  			...
		  			foreach ($affilations as $affilation) {
		  				...
		  				if(Auth::user()->id == $affilation->usersId) {
	  						...
	  						if(!is_null($affilation->co_user) && $affilation->co_user->rolesId==4 && !is_null($affilation->co_school)) {

	  							$affilation_name = $affilation->co_school->school;
	  							$affilation_email = $affilation->co_school->email;

	  						}

	  						else if(!is_null($affilation->co_user) && !is_null($affilation->co_affilation)) {

	  							$affilation_name = $affilation->co_affilation->company;
	  							$affilation_email = $affilation->co_affilation->email;
		  					...
		  					if(!is_null($affilation->user) && $affilation->user->rolesId==4 && !is_null($affilation->school)) {

		  						$affilation_name = $affilation->school->school;
	  							$affilation_email = $affilation->school->email;

		  					}

		  					else if(!is_null($affilation->user) && !is_null($affilation->affilation)) {

		  						$affilation_name = $affilation->affilation->company;
	  							$affilation_email = $affilation->affilation->email;
		  				...
		  				$subject2 = "Application for ".$job_name.".";
		  				$message2 = "Hi! Thank you for your attention and interest on the profile of this <b>".$receiver_name."</b>.<br><br>May we remind you about the existing rules of the Philippine Overseas Employment Administration (POEA), to recruit Filipino job candidates only through a licensed Philippine recruitment agency.<br><br>To be guided on how to proceed with the recruitment process, please contact any of the certified Philippine Sending Organization in this platform.<br><br>Yours truly,<br><b>GCC</b>";
		  				$emailArray2 = array("receiver_email" => $affilation_email, "receiver_name" => $affilation_name,"message" => $message2, "subject" 	=> $subject2);
	    ...
	    public function applicationsByType($type, $id) {
	        ...
	        return view('organization.applicants.summary', compact('applicants', "title"));
	    }
	    ...
	    public function summary() {
	        ...
	        return view('organization.applicants.summary', compact('applicants', 'currencies', 'country', 'specialization'));
	    }
	    ...
	    public function search() {
	        ...
	        return view('organization.applicants.summary', compact('applicants', 'currencies', 'country', 'specialization', 'get_spe', 'get_loc', 'get_title', 'get_curr', 'get_salary'));
	    }
	    ...
	    public function invitations() {
            ...
            return view('organization.applicants.invitations', compact('approves', 'users', 'rejects'));
	    }
	}
?>