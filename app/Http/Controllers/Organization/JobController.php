<?php 
	...
	class JobController extends Controller {
    	...
    	public function index() {
			...
			return view('organization.jobs.index', compact('jobs', 'closes', 'archives'));
		}
	    ...
	    public function create() {
	        ...
	        $partners = SubscriberAffilation::where('isActive', 1)->where(function($q) use($usersId) {
	                $q->where('companyId', $usersId)->orWhere('usersId', $usersId);
	            })->paginate(10);

	        $organizationType = EmployerDetail::with("type")->where("usersId", $usersId)->first();
			...
			return view('organization.jobs.create', compact('types', 'specializations', 'countries', 'subscriptionCheck', 'subscriptionEnded', 'subscriptionLimit', 'disabled', 'completeDetails', 'positions', 'currencies', "partners", "organizationType"));
	    }
	    ...
	    public function store(Request $request) {
	    	...
	    	try {
	            ...
	            if ($partners!==null) {
	                $partner                = new EmployerJobOrder();
	                $partner->jobId         = $school->id;
	                $partner->usersId       = $usersId;
	                $partner->partnersId    = $partners;
	                $partner->save();
	            }
	            ...
	            return redirect('organization/jobs');
	    	} 
	    ...
	    public function edit($id) {
	    	...
	    	return view('organization.jobs.create', compact('types', 'specializations', 'countries', 'jobs' , 'disabled', 'subscriptionEnded', 'positions' , 'currencies', 'partners', 'organizationType'));
	    }
	    ...
	    public function update(Request $request, $id) {
	    	...
	    	try {
	            ...
	            $partners = $request->input("partners");
                $parameters = array('jobId' => $id, 'usersId' => $usersId);

	            if ($partners!==null) {
	                $partner = EmployerJobOrder::firstOrNew($parameters);
	                $partner->jobId      = $id;
	                $partner->usersId    = $usersId;
	                $partner->partnersId = $partners;
	                $partner->save();
	    ...
	    public function summary() {
	        ...
	        return view('organization.jobs.summary', compact("jobs"));
	    }
	}	
?>