...
@section('title', App\MaintenanceLocale::getLocale(507))
     ...
     <div class="col-md-12">
      ...
      <div class="card shadow-none mt-5">
        <div class="card-header">
          <h6 style="float:left; margin-top:10px">{{ App\MaintenanceLocale::getLocale(507) }}</h6>
              ...
              <table class="table table-bordered" id="dataTable">
                ...
                <tbody>
                  ...
                  <tr class="data-row">
                    ...
                    <td>
                      @if($type->isAll==1)
                        Send to all affilations
                      @else
                        Send to specific affilations
                      @endif                      
                    </td>
                    <td class="status" hidden>{{$type->isAll}}</td>
                    <td>
                      <form action="/admin/types/{{ $type->id }}" method="POST" onsubmit="return confirm('{{ App\MaintenanceLocale::getLocale(151) }}')">
                        @csrf
                        @method('DELETE')
                        <button title="{{ App\MaintenanceLocale::getLocale(36) }}" class="fa fa-edit fa-2x" type="button" style="background:none; border: none; color:green" id="edit-item" data-item-id="{{ $type->id }}"></button>
...
<div class="modal fade" id="edit-modal" tabindex="-1" role="dialog" aria-labelledby="edit-modal-label" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form method="POST" id="edit-form" action="{{ route('admin.types.store') }}">
        @csrf
        <div class="modal-body" id="attachment-body-content">
          ...
          <div class="form-check">
              <input type="checkbox" class="form-check-input" id="modal-input-status" name="isAll">
              <label class="form-check-label" for="exampleCheck1">Send to all affilations</label>
          </div>
    ...
    <div class="modal-content">
      ...
      <form method="POST" action="{{ route('admin.types.store') }}">
        ...
        <div class="modal-body" id="attachment-body-content">
          ...
          <div class="form-group">
            <label class="col-form-label" for="modal-input-name">Role</label>
          ...
          <div class="form-check">
              <input type="checkbox" class="form-check-input" id="isAll" name="isAll">
              <label class="form-check-label" for="exampleCheck1">Send to all affilations</label>
          </div>
