			...
			<div class="col-sm-9">
				@if(!isset($title))
					<div class="card card-body">
						<form method="POST" action="{{ url('employer/summary/applicants/search') }}">
							@csrf
							<div class="row">
								<div class="col-sm-4">
					...
					<div class="card page-contents">
						<div class="card-body page-content-body">
							<div class="content-header-1">
								@if($canViewProfile > 0)
									<a href="{{url("employer/applicant/profile/".$applicant->applicant->id) }}" class="content-title" target="_blank"> 
										{{ $applicant->applicant->firstName }} {{ $applicant->applicant->lastName }}
									</a>
								@else
									<label class="content-title">{{ $applicant->applicant->firstName }} {{ $applicant->applicant->lastName }}</label>
								@endif
									...
									@if(!is_null($applicant->applicant))
										@if(!is_null($applicant->applicant->certificates()))
											...
											@foreach($applicant->applicant->certificates() as $certificate)
												@php
													$certificate 	= "<a target='_blank' href='".url($certificate->path.$certificate->filename)."'>".$certificate->filename."</a>";
