...
@section('content')
					...
					<div class="card-body page-content-body">
						<div class="content-header-1">
							<div class="row">
								<div class="col-sm-9">
									@if($canViewProfile > 0)
										<a href="{{url("employer/applicant/profile/".$user->applicant->id) }}" class="content-title" target="_blank"> 
									...
									@if(is_null($user->response))
										@if($user->status==1)
											<p class="content-right">
												<label class="label-blue schedule-applicant rowSchedule{{$user->id}}" name="{{ $user->applicant->firstName }} {{ $user->applicant->lastName }}" email="{{$user->applicant->email}}" id="{{$user->id}}" job_name="{{ $user->application->title }}" rowSchedule="{{$user->id}}">
	...
	<script> 
			        	...
			        	$.ajax({
			        	    ...
			        	    success: function(response) {
			        			if (response.result === true) {
       		        				...
       		        				if(msg['asubject']!==undefined || msg['dmessage']!==undefined || msg['date']!==undefined) {
		...
		$(".premium-decline-applicant").click(function() {
			...
			$(".premium-close-button").after(premium_decline_applicant(name, email, id, job_name, rowDecline));
		});
