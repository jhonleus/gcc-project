										...
										@if($canPostBlog > 0)
											<div class="col-sm-5">
												<p class="content-right">
													<form style="float:right;" action="/employer/blogs/{{ $blog->id }}" method="POST" onsubmit="return confirm('{{ App\MaintenanceLocale::getLocale(151) }}')">
														@csrf
														@method('DELETE')
														<a class="label-blue label-button" href="{{ url('employer/blogs/'.$blog->id.'/edit') }}">
