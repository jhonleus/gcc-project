...
@section('content')
	...
	<div class="container create-container">
		@if(isset($completeDetails))
			@if(!$completeDetails)
				<div class="alert alerts alert-warning" role="alert">
					Please complete your details <a href="{{ url('employer/details/'.Auth::user()->id.'/edit') }}">here</a> before posting a job!
				</div>
			@else
		...
		@if(isset($jobs))
		<form method="POST" action="{{ route('employer.jobs.update', $jobs->id) }}" id="form">
            @method('PUT')
		    @csrf
		@else
		<form action="{{ route('employer.jobs.store') }}" method="POST" id="form" enctype="multipart/form-data">
		    @csrf
		@endif
		<div class="row">
	...
	<script>
		...
		$("#save_button").click(function(e) {
			...
			if(disabled==="true") {
				...
				if(job_title===null || job_title==="" || type===null || type==="" || specialization===null || specialization==="" || ewankoba===null || ewankoba==="" || !description || position===null || position==="" || !responsibility || !qualification || max===null || max==="" || min===null || min==="" || currency===null || currency==="" || !min.match(/^\d+$/) ||!max.match(/^\d+$/) || job_order===0 || city===null || city==="") {
					e.preventDefault();
