		...
		<div class="row">
			...
			<div class="col-sm-8">
				<div class="card sidebar-settings">
					...
					<div class="card-body">
						@if(isset($blogs))
					<form method="POST" action="{{ route('school.blogs.update', $blogs->id) }}" enctype="multipart/form-data">
						@method('PUT')
						@csrf

						<div class="contents-form">
							<h1 class="label-select">Current Cover Photo:</h1>
							<img src="{{ asset('blogs/'.$blogs->filename) }}" style="height: 200px" class="label-image">
						</div>
					@else
					<form method="POST" action="{{ route('school.blogs.store') }}" enctype="multipart/form-data">  
						@csrf
					@endif
            			<div class="contents-form">
						...
						<div class="contents-form">
	                        <div class="row">
								...
								<h1 class="label-select">From</h1>
		                            <select class="select @if(isset($blogs)) select-selected @endif" name="type" required="required">
						...
						<div class="contents-form">
							...
							<div class="ckeditor-news">
								<textarea class="txtarea" name="content" id="content">@if(isset($blogs)){{$blogs->content}}@endif</textarea>
							</div>
